...
export class AIEngine {
  ...
  constructor(options: ForgeAIOptions) {
    ...
    this.conversations = new ConversationManager(options.persistence?.enabled);
    this.settings = new SettingsManager();
    this.rag = new RAGManager(options.rag?.topK, options.rag?.minSimilarity);
    ...
    if (this.ragEnabled && options.rag?.embedModel) {
      ...
      if (p && 'embed' in p) {
        this.rag.setEmbedFunction((text) =>
          (p as OpenAICompatProvider).embed(text, model),
        );
      }
    }
  }
  ...
  private _initProviders(options: ForgeAIOptions): void {
    ...
    for (const custom of p.custom ?? []) {
      const {
        OpenAICompatProvider: OAP,
      } = require('./providers/OpenAICompatProvider');
  ...
  async chatInConversation(
    ...
    const systemPrompt = await this._resolveSystemPrompt(
      options.systemPrompt ?? conv.systemPrompt ?? undefined,
      scope,
    );
    const messages = await this._buildMessages(
      prompt,
  ...
  private async _resolveModel(
    override: ModelString | undefined,
    scope: EngineScope,
  ): Promise<ModelString> {
    if (override) return override;
    return (
      (await this.settings.resolve('model', this.opts.defaultModel, scope)) ??
