...
class AIEngine {
    constructor(options) {
        ...
        // ── Managers ──────────────────────────────────────────────────────────
        this.providers = new ProviderRegistry_1.ProviderRegistry();
        this.conversations = new ConversationManager_1.ConversationManager(options.persistence?.enabled);
        this.settings = new SettingsManager_1.SettingsManager();
        this.rag = new RAGManager_1.RAGManager(options.rag?.topK, options.rag?.minSimilarity);
        this.mcp = new MCPManager_1.MCPManager(options.mcp?.servers);
        this.ragEnabled = options.rag?.enabled ?? false;
        this.mcpAutoInject = options.mcp?.autoInject ?? true;
        // ── Router ────────────────────────────────────────────────────────────
        this.router = new ModelRouter_1.ModelRouter(this.providers, options.routes, options.fallback);
        // ── Providers ─────────────────────────────────────────────────────────
        ...
        if (this.ragEnabled && options.rag?.embedModel) {
            ...
            if (p && 'embed' in p) {
                this.rag.setEmbedFunction((text) => p.embed(text, model));
            }
        }
    }
    _initProviders(options) {
        ...
        for (const custom of p.custom ?? []) {
            ...
            this.providers.register(new OAP({
                id: custom.id,
     ...
     */
    async chat(prompt, scope = {}, options = {}) {
        const model = await this._resolveModel(options.model, scope);
        const systemPrompt = await this._resolveSystemPrompt(options.systemPrompt, scope);
        const messages = await this._buildMessages(prompt, [], systemPrompt, options, scope);
        ...
        const response = await this.router.chatWithFallback(candidates, messages, {
            ...
            temperature: options.temperature ??
                (await this.settings.resolve('temperature', this.opts.temperature, scope)) ??
                this.opts.temperature,
            maxTokens: options.maxTokens ??
                (await this.settings.resolve('maxTokens', this.opts.maxTokens, scope)) ??
                this.opts.maxTokens,
        });
        return {
     ...
     */
    async chatInConversation(prompt, convID, scope = {}, options = {}, autoCreate = true) {
        let conv = null;
        if (convID) {
            conv = await this.conversations.get(convID);
            if (!conv)
                throw new Error(`[ForgeAI] Conversation "${convID}" not found`);
        }
        else if (autoCreate) {
            const existing = await this.conversations.findByScope(scope);
            conv = existing ?? (await this.conversations.create(scope));
        }
        if (!conv)
            throw new Error('[ForgeAI] No conversation ID and autoCreate is false');
        const model = options.model ??
            conv.model ??
            (await this._resolveModel(undefined, scope));
        const systemPrompt = await this._resolveSystemPrompt(options.systemPrompt ?? conv.systemPrompt ?? undefined, scope);
        const messages = await this._buildMessages(prompt, conv.messages, systemPrompt, options, scope);
        const maxHistory = conv.maxTokens ??
            (await this.settings.resolve('maxHistory', this.opts.maxHistory, scope)) ??
        ...
        // Persist messages
        await this.conversations.addMessage(conv.id, { role: 'user', content: prompt, timestamp: Date.now() }, maxHistory);
        await this.conversations.addMessage(conv.id, {
            role: 'assistant',
            content: response.content,
            model: response.usedModel,
            timestamp: Date.now(),
    ...
    // ─── Message building ─────────────────────────────────────────────────────
    async _buildMessages(prompt, history, system, options, scope) {
        const messages = [];
        // System prompt
        if (system)
            messages.push({ role: 'system', content: system, timestamp: 0 });
        // RAG injection
        if ((options.useRAG ?? this.ragEnabled) && this.ragEnabled) {
            const results = await this.rag.search(prompt, options.ragNamespace);
            if (results.length) {
                const context = RAGManager_1.RAGManager.formatContext(results);
    ...
    // ─── Resolution helpers ───────────────────────────────────────────────────
    async _resolveModel(override, scope) {
        if (override)
            return override;
        return ((await this.settings.resolve('model', this.opts.defaultModel, scope)) ??
            this.opts.defaultModel);
    }
    async _resolveSystemPrompt(override, scope) {
        if (this.opts.systemPromptMode === 'append') {
            const parts = [];
            if (this.opts.defaultSystemPrompt)
                parts.push(this.opts.defaultSystemPrompt);
            if (scope.guildID) {
                const gs = await this.settings.getGuild(scope.guildID);
                if (gs.systemPrompt)
                    parts.push(gs.systemPrompt);
                if (scope.channelID &&
                    gs.channelConfig[scope.channelID]?.systemPrompt) {
                    parts.push(gs.channelConfig[scope.channelID].systemPrompt);
                }
            }
            if (scope.userID) {
                const us = await this.settings.getUser(scope.userID);
                if (us.systemPrompt)
                    parts.push(us.systemPrompt);
            }
            if (override)
                parts.push(override);
            return parts.join('\n\n');
        ...
        if (override)
            return override;
        const resolved = await this.settings.resolve('systemPrompt', this.opts.defaultSystemPrompt, scope);
