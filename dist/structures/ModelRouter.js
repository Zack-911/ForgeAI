...
class ModelRouter {
    constructor(registry, routes = {}, fallback = []) {
        ...
        if (!this.routes.has('fast'))
            this.routes.set('fast', []);
        if (!this.routes.has('smart'))
            this.routes.set('smart', []);
        if (!this.routes.has('local'))
            this.routes.set('local', []);
        if (!this.routes.has('cheap'))
            this.routes.set('cheap', []);
        if (!this.routes.has('vision'))
            this.routes.set('vision', []);
    }
    /**
     * Resolve a model string or route name into a concrete `provider:model`.
     * Route names (e.g. "fast", "smart") expand to their model list.
     */
    resolveModels(modelString, defaultModel) {
        // Named route
    ...
    async chatWithFallback(models, messages, options) {
        const candidates = [...new Set([...models, ...this.globalFallback])];
        const errors = [];
        for (const model of candidates) {
            try {
                ...
                forgescript_1.Logger.warn(`[ForgeAI] Model "${model}" failed: ${msg}`);
